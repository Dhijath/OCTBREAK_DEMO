...
--------------------------------------------------------------------------------
   背景 + "GAME OVER"（TextLogo）+ スコア（DirectWrite）
   入力は Game_Manager 側。
==============================================================================*/
#include "Result.h"
#include "texture.h"
...
static DirectWrite* g_pDWLabel = nullptr;   // "SCORE" ラベル（24pt）
...
static constexpr float PNL_H   = 180.0f;
...
void Result_Draw()
{
    ...
    if (g_pDWLabel && g_pDWScore)
    {
        ...
        g_pDWScore->DrawAt(scoreBuf, SCR_CX, PNL_Y + 90.0f, 500.0f, dWHITE, 1.0f);
        g_pDWScore->EndBatch();
