...
{                                                        // ここから
    ...
    //==========================================================================
    XMFLOAT3 gSpectatorPos   = { 0.0f, 2.0f, 0.0f };   // ワールド位置
    float    gSpectatorYaw   = 0.0f;                     // ヨー（左右回転）
    float    gSpectatorPitch = 0.0f;                     // ピッチ（上下回転）
    constexpr float SPEC_MOVE_SPEED = 10.0f;             // 移動速度（m/s）

    //==========================================================================
    // 切替キー（O） 立ち上がり検出用
    //==========================================================================
    bool gPrevToggle = false;                            // 前フレームのO状態

    //==========================================================================
    // パッド ロックオンアシスト
    //==========================================================================
    bool       gAssistEnabled = false;
    XMFLOAT3   gAssistTarget  = { 0.0f, 0.0f, 0.0f };
}                                                        // namespace終わり
...
// 内部：スペクテイターモードへ
//==============================================================================
static void EnterSpectator()                             // スペクテイターへ切替
{                                                        // 開始
    g_Mode = PLAYER_CAMERA_MODE_SPECTATOR;               // モード設定
    // 現在のカメラ位置・向きを引き継いで開始
    gSpectatorPos   = g_Pos;                             // 今の位置から開始
    gSpectatorYaw   = atan2f(g_Front.x, g_Front.z);     // 現在の向きからYaw
    gSpectatorPitch = asinf(                             // 現在の向きからPitch
        std::max(-1.0f, std::min(1.0f, g_Front.y)));
    Mouse_SetMode(MOUSE_POSITION_MODE_RELATIVE);         // 相対入力へ
    Mouse_SetVisible(false);                             // カーソル非表示
    FlushMouseModeEventOnce();                           // 切替イベント即確定
    Mouse_State ms{};                                    // マウス状態
...
{                                                        // 開始
    ...
    switch (mode)                                        // 指定モードへ
    {                                                    // 開始
    case PLAYER_CAMERA_MODE_FIXED_FOLLOW: EnterFixed();     break; // 固定追従へ
    case PLAYER_CAMERA_MODE_MOUSE_FREE:   EnterMouse();     break; // マウスへ
    case PLAYER_CAMERA_MODE_SPECTATOR:    EnterSpectator(); break; // スペクテイターへ
    }                                                    // 終了
}                                                        // 終了

void Player_Camera_ToggleMode()                          // モード切替（3択サイクル）
{                                                        // 開始
    switch (g_Mode)                                      // 現在モードに応じて
    {                                                    // 開始
    case PLAYER_CAMERA_MODE_FIXED_FOLLOW: EnterMouse();      break; // 固定→マウスへ
    case PLAYER_CAMERA_MODE_MOUSE_FREE:   EnterSpectator();  break; // マウス→スペクテイターへ
    case PLAYER_CAMERA_MODE_SPECTATOR:    EnterFixed();      break; // スペクテイター→固定へ
    }                                                    // 終了
}                                                        // 終了
...
{                                                        // 開始
    ...
    if (gAssistEnabled)
    {
        const float stickMag   = sqrtf(rx * rx + ry * ry);          // スティック入力量 0〜1
        const float assistScale = std::max(0.0f, 1.0f - stickMag * 2.0f); // 入力が大きいほど弱く

        // カメラ基準点 → ターゲットへの方向
        XMFLOAT3 pf = Player_GetPosition();
        const float centerY = pf.y + (Player_GetHeight() * 0.5f);
        XMVECTOR pivot  = XMVectorSet(pf.x, centerY + gHeight, pf.z, 0.0f);
        XMVECTOR toTgt  = XMLoadFloat3(&gAssistTarget) - pivot;
        float    dist   = XMVectorGetX(XMVector3Length(toTgt));

        if (dist > 0.1f)
        {
            // ターゲット方向の理想 Yaw / Pitch を算出
            XMFLOAT3 d;
            XMStoreFloat3(&d, toTgt / dist);

            float targetYaw   = atan2f(d.x, d.z);
            float targetPitch = asinf(std::max(-1.0f, std::min(1.0f, d.y)));

            // Yaw は最短経路で補間（±π ラップ対応）
            float dyaw = targetYaw - gYaw;
            while (dyaw >  XM_PI) dyaw -= XM_2PI;
            while (dyaw < -XM_PI) dyaw += XM_2PI;

            constexpr float ASSIST_SPEED = 2.5f;  // rad/sec（緩やかに追尾）
            const float step = ASSIST_SPEED * assistScale * static_cast<float>(elapsed_time);

            gYaw   += std::clamp(dyaw,               -step, step);
            gPitch += std::clamp(targetPitch - gPitch, -step, step);
        }
    }
    // ─────────────────────────────────────────────────────────
...
//==============================================================================
// 内部：スペクテイターカメラ更新（プレイヤー完全独立）
//==============================================================================
static void UpdateSpectatorCamera(double elapsed_time)   // スペクテイターを更新
{                                                        // 開始
    const float dt = static_cast<float>(elapsed_time);   // float変換

    // ── マウス入力 ──────────────────────────────────────────────────────────
    Mouse_State ms{};                                    // マウス状態
    Mouse_GetState(&ms);                                 // マウス取得（相対想定）

    if (ms.positionMode != MOUSE_POSITION_MODE_RELATIVE) // 相対でなければ
    {                                                    // 開始
        Mouse_SetMode(MOUSE_POSITION_MODE_RELATIVE);     // 相対へ
        Mouse_SetVisible(false);                         // 非表示へ
        FlushMouseModeEventOnce();                       // イベント確定
    ...
    // パッド右スティック
    float rx = 0.0f, ry = 0.0f;                          // 右スティック入力
    PadLogger_GetRightStick(&rx, &ry);                   // 取得
    gSpectatorYaw   += rx * 3.0f * dt;                   // パッドでヨー加算
    gSpectatorPitch += ry * 2.0f * dt;                   // パッドでピッチ加算

    // マウスで視点回転（感度は共通変数を使用）
    gSpectatorYaw   +=  ms.x * gMouseSensYaw;            // マウスXでヨー加算
    gSpectatorPitch -= (ms.y * gMouseSensPitch)          // マウスYでピッチ加算
                     * (gMouseInvertY ? -1.0f : 1.0f);   // Y軸反転対応

    constexpr float PITCH_LIMIT = XMConvertToRadians(89.0f); // ピッチ上限
    if (gSpectatorPitch >  PITCH_LIMIT) gSpectatorPitch =  PITCH_LIMIT;
    if (gSpectatorPitch < -PITCH_LIMIT) gSpectatorPitch = -PITCH_LIMIT;

    // ── 前方向・右方向ベクトル計算 ─────────────────────────────────────────
    const float sy = sinf(gSpectatorYaw),   cy = cosf(gSpectatorYaw);
    const float sp = sinf(gSpectatorPitch), cp = cosf(gSpectatorPitch);

    XMVECTOR forward = XMVectorSet(sy * cp, sp, cy * cp, 0.0f); // 前方（YawPitch考慮）
    XMVECTOR right   = XMVectorSet(cy,      0.0f, -sy,   0.0f); // 右方（水平のみ）
    const XMVECTOR worldUp = XMVectorSet(0.0f, 1.0f, 0.0f, 0.0f); // ワールドUp

    // ── WASD + Q/E で移動 ─────────────────────────────────────────────────
    XMVECTOR pos = XMLoadFloat3(&gSpectatorPos);         // 現在位置をロード

    if (KeyLogger_IsPressed(KK_U)) pos += forward  * SPEC_MOVE_SPEED * dt; // 前進
    if (KeyLogger_IsPressed(KK_J)) pos -= forward  * SPEC_MOVE_SPEED * dt; // 後退
    if (KeyLogger_IsPressed(KK_H)) pos -= right    * SPEC_MOVE_SPEED * dt; // 左移動
    if (KeyLogger_IsPressed(KK_K)) pos += right    * SPEC_MOVE_SPEED * dt; // 右移動
    if (KeyLogger_IsPressed(KK_I)) pos += worldUp  * SPEC_MOVE_SPEED * dt; // 上昇
    if (KeyLogger_IsPressed(KK_N)) pos -= worldUp  * SPEC_MOVE_SPEED * dt; // 下降

    XMStoreFloat3(&gSpectatorPos, pos);                  // 位置を保存

    // ── View/Proj 構築・適用 ────────────────────────────────────────────────
    XMVECTOR eye    = pos;                               // 視点
    XMVECTOR target = pos + XMVector3Normalize(forward); // 注視点（前方1単位）

    XMStoreFloat3(&g_Front, XMVector3Normalize(forward)); // 前方向を保存
    XMStoreFloat3(&g_Pos, eye);                           // 位置を保存

    XMMATRIX view = XMMatrixLookAtLH(eye, target, worldUp); // View行列
    XMMATRIX proj = MakeMainProjection();                // Proj行列
...
{                                                        // 開始
    ...
    // カメラモード切替（Oキー：FIXED→MOUSE→SPECTATOR→FIXED の3択サイクル）
    const bool nowToggle = KeyLogger_IsPressed(KK_O);    // O押下状態
    ...
    else if (g_Mode == PLAYER_CAMERA_MODE_SPECTATOR)     // スペクテイターなら
        UpdateSpectatorCamera(elapsed_time);             // スペクテイター更新
    else                                                 // それ以外（固定追従）
...
//==============================================================================
void Player_Camera_SetLockOnAssist(const XMFLOAT3* targetWorldPos)
{
    gAssistEnabled = (targetWorldPos != nullptr);
